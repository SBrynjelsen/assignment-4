...
5.  Når dere benytter `right_join()` til å sette sammen dataframes til en stor dataframe benytter dere ikke `by` argumentet. Vi vil da ha match på knr, knavn, aar, mnd. Skulle knavn være ørlite forskjellig vil vi ikke få match og vi får en feil som kan være vanskelig å finne. Et godt råd er derfor å sjekke om knavn er likt i de tre dataframene. Jeg har lagt inn slik kode rett før `join` rundt linje 260 i `make-data-tidy.Rms`.

6.  I model.Rmd ca. linje 130 setter dere fargen (colour =) inne i `aes()` funksjonen. Dette betyr at ggplot skal sette farge på hver kategori vha. en kategorivariabel. Når dere selv vil styre fargene spesifiserer dere fargen *utenfor* `aes()`. Altså `geom_line(mapping = aes(y = wf_M/bef_M),  color = "blue")` og *ikke* `geom_line(mapping = aes(y = wf_M/bef_M, color = "darkblue"))`
